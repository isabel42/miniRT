...
/*   Updated: 2023/11/06 14:58:41 by itovar-n         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
t_rgb	get_texture_from_plane(t_hit *hit, t_data_img *texture, t_vec3d cam)
{
	...
	v_plan = ft_v_cprod(hit->normal, (t_vec3d){1, 0, 0});
	if (!ft_mod(v_plan))
	{
		v_plan = ft_v_cprod(hit->normal, (t_vec3d){0, 1, 0});
		if (!ft_mod(v_plan))
			v_plan = ft_v_cprod(hit->normal, (t_vec3d){0, 0, 1});
		if (!ft_mod(v_plan))
			return (int_to_rgb(get_color_from_texture(texture, 0, 0)));
	}
	x = get_image_coord(hit, texture->height, closest, v_plan);
	v_plan = ft_v_cprod(hit->normal, v_plan);
